...
{
    for (int i = 0; i < candidate_count; i++)
        if (strcasecmp(name,candidate_count) == 0)
        {
            ranks[rank] = i;
            return true;
        }
...
void record_preferences(int ranks[])
{
    //Setting all values in preferences to 0
    for (int i = 0; i < MAX; i++)
        for (int j = 0; j < MAX; j++)
        preferences[i][j] = 0;
    //Rzecording preferences
    for (int i = 0; i < candidate_count; i++)
    ...
    for (int i = 0; i < pair_count; i++)
        for (int j = i+1; j < pair_count; j++)
            if(preferences[pairs[j].winner][pairs[j].loser] > preferences[pairs[i].winner][pairs[i].loser])
            {
                ...
                preferences[pairs[j].winner][pairs[j].loser] = preferences[pairs[j].winner][pairs[j].loser];
